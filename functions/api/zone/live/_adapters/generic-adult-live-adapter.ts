/**
 * Generic adult live streaming source adapter factory.
 *
 * Creates stub adapters for adult live streaming sources that share the same
...
export interface GenericAdultLiveAdapterOptions {
  id: string;
  name: string;
  priority: number;
  searchUrl: string;
...
export function createGenericAdultLiveConfig(
  opts: GenericAdultLiveAdapterOptions,
...
export class GenericAdultLiveAdapter extends BaseSourceAdapter {
  private platform: string;

  constructor(opts: GenericAdultLiveAdapterOptions, configOverrides?: Partial<SourceConfig>) {
    super(createGenericAdultLiveConfig(opts, configOverrides));
  ...
  async getDetail(itemId: string): Promise<AggregatedItem | null> {
    return this.buildItem({
      ...
      cover: '',
      url: `/api/zone/live/stream/${itemId}`,
      metadata: {
        platform: this.platform,
        description: `${this.config.name} live stream detail (stub)`,
        isLive: true,
      },
    });
  }
  ...
  async getStreamUrl(itemId: string): Promise<string> {
    ...
    return `https://cf-proxy.workers.dev/zone/live/${this.config.id}/${itemId}`;
  }
