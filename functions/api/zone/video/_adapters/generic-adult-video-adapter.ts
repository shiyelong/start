/**
 * Generic adult video source adapter factory.
 *
 * Creates stub adapters for adult video sources that share the same
...
export interface GenericAdultVideoAdapterOptions {
  id: string;
  name: string;
  priority: number;
  searchUrl: string;
...
export function createGenericAdultVideoConfig(
  opts: GenericAdultVideoAdapterOptions,
...
export class GenericAdultVideoAdapter extends BaseSourceAdapter {
  private platform: string;

  constructor(opts: GenericAdultVideoAdapterOptions, configOverrides?: Partial<SourceConfig>) {
    super(createGenericAdultVideoConfig(opts, configOverrides));
  ...
  async search(query: string, page: number, pageSize: number): Promise<AggregatedItem[]> {
    ...
    for (let i = 0; i < count; i++) {
      items.push(this.buildItem({
        ...
        cover: '',
        url: `/api/zone/video/stream/${this.config.id}-${query}-${i}`,
        metadata: {
  ...
  async getDetail(itemId: string): Promise<AggregatedItem | null> {
    return this.buildItem({
      ...
      cover: '',
      url: `/api/zone/video/stream/${itemId}`,
      metadata: {
        platform: this.platform,
  ...
  async getStreamUrl(itemId: string): Promise<string> {
    ...
    return `https://cf-proxy.workers.dev/zone/video/${this.config.id}/${itemId}`;
  }
