...
/** Convert "#rrggbb" hex string to numeric color */
...
export default function ForestAdventurePage() {
  ...
  useEffect(() => {
    ...
    async function initPixi() {
      ...
      if (destroyed) return;
      const app = await createPixiApp({ canvas: canvas!, width: CANVAS_W, height: CANVAS_H, backgroundColor: 0x000000, antialias: true });
      ...
      app.ticker.add(() => {
        ...
        // ── Parallax Background ──
        // Sky gradient (approximate with rects)
        g.rect(0, 0, CANVAS_W, CANVAS_H * 0.5).fill({ color: 0x87CEEB });
        g.rect(0, CANVAS_H * 0.5, CANVAS_W, CANVAS_H * 0.25).fill({ color: 0xb8e6b8 });
        g.rect(0, CANVAS_H * 0.75, CANVAS_W, CANVAS_H * 0.25).fill({ color: 0x5a8a3a });

        level.bgLayers.forEach(layer => {
          const offset = camX * layer.speed;
          const layerColor = cn(layer.color);
          layer.elements.forEach(el => {
            const ex = ((el.x - offset) % (CANVAS_W + 200)) - 100;
            const ey = el.y;
            if (el.type === "cloud") {
              g.circle(ex, ey, 40 * el.scale).fill({ color: layerColor, alpha: 0.6 });
              g.circle(ex + 25, ey + 5, 30 * el.scale).fill({ color: layerColor, alpha: 0.6 });
            } else if (el.type === "mountain") {
              g.poly([ex - 80 * el.scale, CANVAS_H, ex, ey, ex + 80 * el.scale, CANVAS_H]).fill({ color: layerColor });
            } else if (el.type === "tree") {
              // Trunk
              g.rect(ex + 8, ey, 8 * el.scale, 40 * el.scale).fill({ color: 0x5a3a1a });
              // Canopy
              g.poly([ex - 10 * el.scale, ey + 10, ex + 12, ey - 25 * el.scale, ex + 34 * el.scale, ey + 10]).fill({ color: layerColor });
            } else if (el.type === "bush") {
              g.circle(ex + 15, ey + 10, 20 * el.scale).fill({ color: layerColor });
            }
          });
        });
        ...
        level.platforms.forEach(pl => {
          ...
          if (pl.type === "solid") {
            g.rect(sx, pl.y, pl.width, pl.height).fill({ color: 0x5a3a1a });
            g.rect(sx, pl.y, pl.width, 6).fill({ color: 0x3a8a2a });
            // Grass tufts
            for (let i = 0; i < pl.width; i += 20) {
              g.rect(sx + i + 4, pl.y - 3, 3, 5).fill({ color: 0x4a9a3a });
              g.rect(sx + i + 10, pl.y - 2, 2, 4).fill({ color: 0x4a9a3a });
            }
          } else {
            g.rect(sx, pl.y, pl.width, pl.height).fill({ color: 0x7a5a3a });
            g.rect(sx, pl.y, pl.width, 4).fill({ color: 0x9a7a5a });
            // Vine decoration
            for (let i = 10; i < pl.width; i += 30) {
              g.moveTo(sx + i, pl.y + pl.height).lineTo(sx + i + 5, pl.y + pl.height + 10).stroke({ color: 0x3a7a2a, width: 1 });
            }
          }
        });
        ...
        gs.items.forEach(item => {
          ...
          switch (item.type) {
            case "heal":
              ...
              break;
            case "atkBoost":
              g.poly([sx + 8, item.y + bob - 4, sx + 14, item.y + bob + 6, sx + 2, item.y + bob + 6]).fill({ color: 0xffaa00 });
              break;
            case "shield":
              g.poly([sx + 8, item.y + bob - 4, sx + 16, item.y + bob + 2, sx + 8, item.y + bob + 12, sx, item.y + bob + 2]).fill({ color: 0x00aaff });
              break;
            case "coin":
              g.circle(sx + 8, item.y + bob + 4, 6).fill({ color: 0xffdd00 });
              showText("$", sx + 5, item.y + bob + 1, { fill: "#aa8800", fontSize: 8 });
              break;
          }
        });
        ...
        gs.enemies.forEach(e => {
          const sx = e.x - camX;
          const eAlpha = e.dead ? Math.max(0, e.deathTimer / 30) : 1;
          const info = ENEMY_TYPES[e.type as keyof typeof ENEMY_TYPES] || ENEMY_TYPES.slime;
          const eColor = cn(info.color);

          if (e.type === "slime") {
            // Slime body (ellipse approximated with circle)
            g.circle(sx + e.width / 2, e.y + e.height - e.height / 4, e.width / 2).fill({ color: eColor, alpha: eAlpha });
            // Eyes
            g.rect(sx + 6, e.y + 4, 4, 4).fill({ color: 0xffffff, alpha: eAlpha });
            g.rect(sx + 14, e.y + 4, 4, 4).fill({ color: 0xffffff, alpha: eAlpha });
            g.rect(sx + 7, e.y + 5, 2, 2).fill({ color: 0x333333, alpha: eAlpha });
            g.rect(sx + 15, e.y + 5, 2, 2).fill({ color: 0x333333, alpha: eAlpha });
          } else if (e.type === "wolf") {
            g.rect(sx, e.y, e.width, e.height).fill({ color: eColor, alpha: eAlpha });
            g.rect(sx + 2, e.y + 2, 8, 6).fill({ color: 0x443322, alpha: eAlpha });
            g.rect(sx + e.width - 10, e.y + 2, 8, 6).fill({ color: 0x443322, alpha: eAlpha });
            g.rect(sx + 6, e.y + 8, 3, 3).fill({ color: 0xff0000, alpha: eAlpha });
            g.rect(sx + e.width - 9, e.y + 8, 3, 3).fill({ color: 0xff0000, alpha: eAlpha });
          } else if (e.type === "bat") {
            g.rect(sx + 8, e.y + 4, 10, 10).fill({ color: eColor, alpha: eAlpha });
            const wingOff = Math.sin(e.flyAngle * 3) * 4;
            g.rect(sx, e.y + 2 + wingOff, 10, 6).fill({ color: eColor, alpha: eAlpha });
            g.rect(sx + 16, e.y + 2 - wingOff, 10, 6).fill({ color: eColor, alpha: eAlpha });
            g.rect(sx + 10, e.y + 7, 2, 2).fill({ color: 0xff0000, alpha: eAlpha });
            g.rect(sx + 14, e.y + 7, 2, 2).fill({ color: 0xff0000, alpha: eAlpha });
          }

          // HP bar
          if (e.hp < e.maxHp && !e.dead) {
            g.rect(sx, e.y - 8, e.width, 4).fill({ color: 0x660000 });
            g.rect(sx, e.y - 8, e.width * (e.hp / e.maxHp), 4).fill({ color: 0xff0000 });
          }
        });
        ...
        if (gs.boss && (gs.boss.deathTimer > 0 || !gs.boss.dead)) {
          ...
          // Body
          const bodyColor = b.name === "Mushroom King" ? 0x8B4513 : 0x2d5a1e;
          g.rect(sx, b.y, b.width, b.height).fill({ color: bodyColor, alpha: bAlpha });
          // Cap/Crown
          const capColor = b.name === "Mushroom King" ? 0xff4444 : 0x1a3a0e;
          g.circle(sx + b.width / 2, b.y, b.width * 0.7).fill({ color: capColor, alpha: bAlpha });
          // Eyes
          g.rect(sx + 10, b.y + 15, 8, 8).fill({ color: 0xffff00, alpha: bAlpha });
          g.rect(sx + b.width - 18, b.y + 15, 8, 8).fill({ color: 0xffff00, alpha: bAlpha });
          g.rect(sx + 12, b.y + 17, 4, 4).fill({ color: 0xff0000, alpha: bAlpha });
          g.rect(sx + b.width - 16, b.y + 17, 4, 4).fill({ color: 0xff0000, alpha: bAlpha });
          // HP bar
          g.rect(sx, b.y - 14, b.width, 6).fill({ color: 0x660000 });
          g.rect(sx, b.y - 14, b.width * (b.hp / b.maxHp), 6).fill({ color: 0xff0000 });
          showText(b.name, sx, b.y - 22, { fill: "#ffffff", fontSize: 10 });
          // Projectiles
          b.projectiles.forEach(proj => {
            if (!proj.active) return;
            g.rect(proj.x - camX, proj.y, proj.width, proj.height).fill({ color: 0xaa00ff });
          });
        }
        ...
        // ── Player ──
        {
          const sx = p.x - camX;
          const sy = p.y;
          const visible = !(p.invincible > 0 && Math.floor(p.invincible / 4) % 2 === 0);
          if (visible) {
            // Body
            g.rect(sx + 4, sy + 8, 20, 20).fill({ color: 0x2d8a4e });
            // Head
            g.rect(sx + 6, sy, 16, 14).fill({ color: 0xf5d0a9 });
            // Eyes
            const eyeX = p.facing === "right" ? sx + 16 : sx + 10;
            g.rect(eyeX, sy + 4, 3, 3).fill({ color: 0x333333 });
            // Legs
            const legOff = p.onGround ? Math.sin(p.animFrame * 0.3) * 3 : 0;
            g.rect(sx + 6, sy + 28, 6, 8 + legOff).fill({ color: 0x665544 });
            g.rect(sx + 16, sy + 28, 6, 8 - legOff).fill({ color: 0x665544 });
            // Attack
            if (p.attacking) {
              const atkX = p.facing === "right" ? sx + PLAYER_W : sx - ATTACK_RANGE;
              g.rect(atkX, sy + 4, ATTACK_RANGE, 20).fill({ color: 0xffff00, alpha: 0.6 });
            }
            // Shield
            if (p.shieldActive) {
        ...
        {
          // HP bar background
          g.rect(10, 10, 154, 20).fill({ color: 0x000000, alpha: 0.5 });
          g.rect(12, 12, 150, 16).fill({ color: 0x660000 });
          const hpPct = p.hp / p.maxHp;
          const hpColor = hpPct > 0.5 ? 0x00cc00 : hpPct > 0.25 ? 0xffcc00 : 0xff0000;
          g.rect(12, 12, 150 * hpPct, 16).fill({ color: hpColor });
          showText(`HP: ${p.hp}/${p.maxHp}`, 16, 13, { fill: "#ffffff", fontSize: 11 });

          // Coins
          g.circle(190, 20, 8).fill({ color: 0xffdd00 });
          showText(`x${p.coins}`, 202, 16, { fill: "#ffffff", fontSize: 12 });

          // Level
          showText(`Level ${level.id}`, 260, 16, { fill: "#ffffff", fontSize: 12 });

          // Score
          showText(`Score: ${p.score}`, 340, 16, { fill: "#ffffff", fontSize: 12 });

          // Time
          showText(`Time: ${Math.floor(gs.time)}s`, 460, 16, { fill: "#ffffff", fontSize: 12 });

          // Atk boost indicator
          if (p.atkBoostTimer > 0) {
            showText(`ATK UP ${Math.ceil(p.atkBoostTimer / 60)}s`, 580, 16, { fill: "#ffaa00", fontSize: 12 });
          }
          if (p.shieldActive) {
            showText("SHIELD", 700, 16, { fill: "#00aaff", fontSize: 12 });
          }
        }
        ...
        gs.notifications.forEach((n, i) => {
          ...
          showText(n.text, CANVAS_W / 2 - 40, 60 + i * 20, { fill: "#ffffff", fontSize: 14, alpha });
        });
      }); // end app.ticker.add
    } // end initPixi
  ...
  const renderUpgrade = () => {
    const labels: Record<string, { name: string; desc: string }> = {
      hp: { name: "Max HP +25", desc: "Increase maximum health" },
      atk: { name: "ATK +5", desc: "Increase attack power" },
      jump: { name: "Jump +1", desc: "Jump higher" },
    };
    return (
