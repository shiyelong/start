...
- **NAS_Cache**: NAS安全缓存层，通过Cloudflare Tunnel加密连接，对缓存内容AES-256加密存储并混淆文件名
...
2. WHEN 用户在聚合搜索框中输入关键词, THE Video_Aggregator SHALL 同时向所有已配置的免费视频源发起搜索并合并结果
3. THE Video_Aggregator SHALL 在搜索结果中标注每个视频的来源站点名称
4. WHEN 用户点击聚合搜索结果中的视频, THE Video_Player SHALL 通过Cloudflare代理在当前页面内播放该视频
...
1. THE Music_Aggregator SHALL 聚合尽可能多的免费音乐源（包括但不限于网易云音乐、QQ音乐、酷狗音乐、酷我音乐、虾米音乐、咪咕音乐、Spotify、SoundCloud、Bandcamp、Jamendo、Free Music Archive、YouTube Music音频提取等公开接口或第三方聚合API），提供统一搜索能力
2. WHEN 用户在音乐搜索框中输入关键词, THE Music_Aggregator SHALL 同时向所有已配置的音乐源发起搜索并合并结果
3. THE Music_Aggregator SHALL 在搜索结果中标注每首歌曲的来源平台名称
4. WHEN 用户点击一首歌曲, THE Music_Player SHALL 通过Cloudflare代理获取音频流并开始播放，同时在页面底部显示迷你播放条
...
1. THE Adult_Video_Adapter SHALL 聚合尽可能多的成人视频网站（包括但不限于Pornhub、XVideos、XNXX、YouPorn、RedTube、Tube8、SpankBang、EPorner、HQPorner、Naughty America、Brazzers免费区、xHamster、ThisAV、JAVHD、Missav、Jable等），通过Cloudflare Workers代理访问
2. WHEN 成人模式用户在成人专区搜索视频, THE Adult_Video_Adapter SHALL 同时向所有已配置的成人视频源发起搜索并合并结果
3. THE Adult_Video_Adapter SHALL 在搜索结果中标注每个视频的来源站点名称
4. WHEN 用户点击成人视频搜索结果, THE Video_Player SHALL 通过Cloudflare代理在当前页面内播放该视频
...
7. THE Adult_Video_Adapter SHALL 支持多标签组合筛选，用户可同时选择多个标签进行精确筛选：
   - 地区/产地：日本AV、欧美、国产、韩国、东南亚、印度、拉美等
   - 类型：剧情片、纯色情、动画/3D、业余自拍、直播录像、偷拍、VR、ASMR等
   - 题材：校园、职场、家庭、户外、制服、角色扮演、SM/BDSM、群交、同性、变装等
   - 演员特征：人种、体型、年龄段等
   - 画质：4K/1080p/720p/480p
   - 时长：短片（<10分钟）、中片（10-30分钟）、长片（>30分钟）、全片电影
   - 排序：热度、最新、评分、播放量、时长
...
1. THE Comic_Aggregator SHALL 聚合尽可能多的主流漫画源（包括但不限于漫画柜、动漫之家、拷贝漫画、包子漫画、奇妙漫画、漫画DB、MangaDex、MangaReader、MangaKakalot、MangaPark、Webtoon、快看漫画、腾讯动漫、有妖气等），提供统一搜索能力
2. WHEN 用户在漫画搜索框中输入关键词, THE Comic_Aggregator SHALL 同时向所有已配置的漫画源发起搜索并合并结果
...
1. THE Adult_Comic_Adapter SHALL 聚合尽可能多的成人漫画网站（包括但不限于E-Hentai、nhentai、Hitomi、Pururin、HentaiNexus、Tsumino、Hentai2Read、MangaHentai、Luscious、HentaiFox、IMHentai等），通过Cloudflare Workers代理访问
2. WHEN 成人模式用户在成人漫画专区搜索, THE Adult_Comic_Adapter SHALL 同时向所有已配置的成人漫画源发起搜索并合并结果
...
7. THE Adult_Comic_Adapter SHALL 支持多标签组合筛选：
   - 类型：纯爱、后宫、触手、NTR、百合、耽美、校园、奇幻、调教、SM、群交、人妻、巨乳、萝莉风等
   - 语言：中文、英文、日文、韩文等
   - 画风：日漫、韩漫、欧美、国漫
   - 排序：热度、最新、评分、收藏数
...
1. THE Video_Aggregator SHALL 接入尽可能多的视频平台（通过Cloudflare Workers代理或嵌入播放器），包括但不限于：
   - Twitch（游戏直播和VOD回放）
   - Dailymotion（海外视频平台）
   - Vimeo（高质量创作者视频）
   - 抖音/TikTok（短视频）
...
1. THE Anime_Aggregator SHALL 聚合尽可能多的动漫资源站（包括但不限于樱花动漫、AGE动漫、OmoFun、Anime1、AnimePahe、GoGoAnime、9Anime、AnimeDao、Zoro.to、Crunchyroll免费区、动漫花园、萌番组、简单动漫等），提供统一搜索能力
2. WHEN 用户在动漫频道搜索, THE Anime_Aggregator SHALL 同时向所有已配置的动漫源发起搜索并合并结果
...
1. THE Novel_Aggregator SHALL 聚合尽可能多的免费小说源（包括但不限于笔趣阁、69书吧、全本小说网、顶点小说、八一中文网、书趣阁、飘天文学、UU看书、小说旗、无错小说网、落秋中文、Novel Updates、Light Novel World、ReadNovelFull等），提供统一搜索能力
2. WHEN 用户在小说搜索框中输入关键词, THE Novel_Aggregator SHALL 同时向所有已配置的小说源发起搜索并合并结果
...
1. THE Podcast_Aggregator SHALL 聚合尽可能多的播客平台（包括但不限于Apple Podcasts、Spotify Podcasts、小宇宙、喜马拉雅、蜻蜓FM、荔枝FM、Google Podcasts、Pocket Casts、Overcast、Castbox、Podcast Addict等）的节目目录
2. WHEN 用户在播客搜索框中输入关键词, THE Podcast_Aggregator SHALL 搜索所有已配置的播客源并合并结果
...
1. THE Live_Aggregator SHALL 聚合尽可能多的直播平台（包括但不限于斗鱼、虎牙、B站直播、Twitch、YouTube Live、抖音直播、快手直播、花椒直播、映客直播、企鹅电竞、CC直播、AfreecaTV、Kick、Facebook Gaming等）的直播间列表
2. WHEN 用户在直播中心浏览, THE Live_Aggregator SHALL 展示各平台正在直播的热门直播间
3. THE Live_Aggregator SHALL 为每个直播间展示封面、主播名称、直播标题、观看人数和平台来源标签
4. WHEN 用户点击直播间, THE Video_Player SHALL 通过Cloudflare代理在当前页面内播放该直播流
...
1. THE Adult_Novel_Adapter SHALL 聚合尽可能多的成人小说网站（包括但不限于禁忌书屋、69书吧成人区、H小说网、成人文学城、Literotica、AO3成人分区、Novelcool成人区等），通过Cloudflare Workers代理访问
2. WHEN 成人模式用户在成人小说专区搜索, THE Adult_Novel_Adapter SHALL 同时向所有已配置的成人小说源发起搜索并合并结果
...
7. THE Adult_Novel_Adapter SHALL 支持多标签组合筛选：
   - 类型：纯爱、后宫、NTR、百合、耽美、校园、奇幻、都市、古代、科幻、调教等
   - 语言：中文、英文、日文等
   - 字数范围：短篇（<5万字）、中篇（5-20万字）、长篇（>20万字）
   - 排序：热度、最新、评分、字数
...
1. THE Adult_Live_Adapter SHALL 聚合尽可能多的成人直播平台（包括但不限于Chaturbate、StripChat、BongaCams、LiveJasmin、CamSoda、MyFreeCams、Flirt4Free等），通过Cloudflare Workers代理访问
2. WHEN 成人模式用户在成人直播专区浏览, THE Adult_Live_Adapter SHALL 展示各平台正在直播的热门直播间
...
7. THE Adult_Live_Adapter SHALL 支持按分类和平台筛选
...
1. THE Adult_Music_Adapter SHALL 聚合含有露骨歌词/内容的音乐源（包括但不限于各平台的Explicit标记歌曲、成人向ASMR音频、成人广播剧、DLsite音声作品、Pornhub音频区等），通过Cloudflare Workers代理访问
2. WHEN 成人模式用户在成人音乐专区搜索, THE Adult_Music_Adapter SHALL 同时向所有已配置的成人音乐源发起搜索并合并结果
...
7. THE Adult_Music_Adapter SHALL 支持多标签组合筛选：
   - 类型：ASMR、成人广播剧、音声作品、Explicit歌曲、成人催眠音频等
   - 语言：中文、英文、日文、韩文等
   - 声优性别：男声、女声、双人、多人
   - 排序：热度、最新、评分、时长
...
1. THE Adult_Game_Adapter SHALL 聚合尽可能多的成人游戏平台和资源站（包括但不限于DLsite、DMM Games、Nutaku、Itch.io成人区、F95Zone、Lewdzone、Newgrounds成人区、成人HTML5游戏站、成人Flash游戏站、成人WebGL游戏站等），通过Cloudflare Workers代理访问
...
8. THE Adult_Game_Adapter SHALL 支持多标签组合筛选，用户可同时选择多个标签精确筛选：
   - 游戏类型：视觉小说、RPG、模拟经营、动作、解谜、卡牌、格斗、射击、策略、养成、换装、沙盒等
   - 题材：纯色情、热血+色情、恋爱+色情、奇幻+色情、校园+色情、后宫、NTR、百合、耽美等
   - 画风：2D动漫、3D写实、像素风、欧美卡通等
   - 语言：中文、英文、日文、韩文等
   - 是否网页可玩：仅看网页游戏 / 全部
   - 排序：热度、评分、最新、随机
...
2. THE Adult_Service_Platform SHALL 为每个服务者/商家展示以下信息：
   ...
   - 服务类型大类和技能标签（结构化分类）：
     - SPA/按摩类：精油按摩、泰式按摩、足疗、全身按摩、头部按摩、热石按摩、淋巴排毒
     - 陪伴类：陪聊、陪玩、陪逛街、陪旅行、陪健身、陪看电影、商务陪同
     - 表演/娱乐类：舞蹈表演、唱歌、DJ、模特、私人派对表演
     - 健康/美容类：瑜伽教练、私人教练、美容护理、美甲
     - 成人服务类：全套服务、半套服务、口交服务、特殊服务、SM/BDSM服务、角色扮演、制服诱惑、双人服务、多人服务
     - 线上服务类：视频聊天、语音陪聊、在线表演、定制内容
   - 特殊技能/卖点描述（自由文本）
...
5. THE Adult_Service_Platform SHALL 支持社区验证机制：已注册用户可对"待验证"服务者提交验证报告（包含真实性评价：照片是否与本人一致、服务描述准确度、安全性评估）
6. WHEN 一个服务者收到至少3个正面验证报告, THE Adult_Service_Platform SHALL 将该服务者状态更新为"已验证"，并在照片上添加验证标记
7. WHEN 一个服务者收到多个负面验证报告或被举报为欺诈, THE Adult_Service_Platform SHALL 将该服务者标记为"警告"或"已标记为欺诈"，并在列表中醒目提示
...
2. THE Adult_Service_Platform SHALL 为求职者提供个人求职档案，包含以下字段：
   ...
   - 语言能力（语种及流利程度）
   - 工作经验描述（从业年限、曾任职场所类型）
   - 技能标签（复用需求36的结构化服务类型分类：SPA/按摩类、陪伴类、表演/娱乐类、健康/美容类、成人服务类、线上服务类）
   - 期望薪资范围
   - 期望工作时间（全职/兼职/弹性）
   - 个人照片（最多10张）
...
2. THE Adult_Social SHALL 支持多个论坛分区：交流区、经验分享区、资源分享区、约会交友区、从业者交流区、安全提醒区
...
2. THE Adult_Social SHALL 为约会档案提供以下字段：昵称、国籍、所在城市、年龄范围、性别、性取向、人种/族裔、身高、体型、兴趣标签、自我介绍、照片（最多6张）
3. THE Adult_Social SHALL 支持按地区/城市、年龄、性别、性取向、人种/族裔和兴趣标签筛选约会对象
4. THE Adult_Social SHALL 支持"喜欢/不喜欢"滑动匹配机制，双方互相喜欢后自动开启聊天
...
1. THE Adult_Anime_Adapter SHALL 聚合尽可能多的成人动漫/里番资源站（包括但不限于Hanime.tv、HentaiHaven、HentaiStream、Ohentai、Hentai.tv、AnimeidhentaI、9hentai等），通过Cloudflare Workers代理访问
2. WHEN 成人模式用户在成人动漫专区搜索, THE Adult_Anime_Adapter SHALL 同时向所有已配置的成人动漫源发起搜索并合并结果
...
6. THE Adult_Anime_Adapter SHALL 支持多标签组合筛选，用户可同时选择多个类型标签（如"热血+色情"、"校园+后宫"、"奇幻+触手"等），按年份、状态、评分和热度排序
...
1. THE Video_Aggregator SHALL 支持接入Telegram公开频道作为视频/图片源，通过Cloudflare Workers代理Telegram Bot API获取频道内容
...
**用户故事:** 作为星聚平台运营者，我希望将热门内容和重要资源安全地缓存到NAS上做永久存储，以提高访问速度并防止源站失效导致内容丢失，同时确保NAS数据不被发现。

#### 验收标准

1. THE Privacy_Shield SHALL 支持通过Cloudflare Tunnel将NAS作为内容缓存层，所有缓存请求经过Cloudflare CDN → Cloudflare Tunnel → NAS，NAS零公网端口
2. THE Download_Manager SHALL 支持管理员将热门视频/音乐/漫画/小说资源下载到NAS本地存储作为永久缓存
3. THE Privacy_Shield SHALL 对NAS上的所有缓存内容进行加密存储（AES-256），即使NAS硬盘被物理获取也无法直接读取内容
4. THE Privacy_Shield SHALL 将NAS缓存内容的文件名和目录结构进行混淆处理，使用随机哈希命名而非原始文件名
5. THE Privacy_Shield SHALL 维护一个加密的索引数据库（存储在NAS上），记录混淆文件名与原始内容的映射关系，该数据库使用独立密钥加密
6. WHEN 用户请求已缓存的内容, THE Cloudflare_Proxy SHALL 优先从NAS缓存返回内容（通过Tunnel），减少对源站的请求
7. WHEN NAS缓存未命中, THE Cloudflare_Proxy SHALL 从源站获取内容并返回给用户，同时异步将内容缓存到NAS
8. THE Privacy_Shield SHALL 支持管理员设置缓存策略：按内容类型设置缓存优先级、最大缓存空间、自动清理策略（LRU/按时间/按访问频率）
9. THE Privacy_Shield SHALL 对NAS的网络流量进行伪装，使运营商无法通过流量特征识别NAS上存储的内容类型：
   - 所有流量通过Cloudflare Tunnel加密传输，运营商只能看到与Cloudflare的加密连接
   - 流量速率限制和随机化，避免大量持续下载引起运营商注意
   - 支持设置每日最大带宽使用量，分散在全天各时段
10. THE Privacy_Shield SHALL 支持NAS缓存数据的紧急销毁功能：管理员可通过远程命令一键删除所有缓存内容和索引数据库
11. THE Privacy_Shield SHALL 支持NAS缓存数据的异地备份：定期将加密的缓存索引备份到Cloudflare R2，NAS损坏时可从R2恢复索引（内容需重新缓存）
12. THE Privacy_Shield SHALL 确保NAS上不存储任何用户个人数据，仅存储内容缓存文件
...
2. THE Admin_Panel SHALL 提供以下管理模块：
   ...
   - NAS缓存管理：查看缓存状态、手动触发缓存/清理、配置缓存策略、紧急销毁
