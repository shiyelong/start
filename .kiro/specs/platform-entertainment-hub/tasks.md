...
- [ ] 17. 视频聚合引擎（前端 + 后端）
  ...
  - [ ] 17.2 实现视频聚合后端 API
    ...
    - 实现免费视频源适配器（至少 3 个初始源）
    - _需求: 2.1, 2.2, 2.5, 2.6, 3.1, 3.2, 3.3, 3.6, 3.8, 4.1, 4.2, 4.3, 4.4, 16.1, 16.2, 16.5, 16.6_
...
- [ ] 18. 音乐聚合引擎（前端 + 后端）
  ...
  - [ ] 18.2 实现音乐聚合后端 API
    ...
    - 实现音乐源适配器（至少 3 个初始源：网易云/QQ音乐/Spotify 等）
...
- [ ] 19. 漫画聚合引擎（前端 + 后端）
  ...
  - [ ] 19.2 实现漫画聚合后端 API
    ...
    - 实现漫画源适配器（至少 3 个初始源：漫画柜/MangaDex/动漫之家等）
...
- [ ] 20. 小说聚合引擎（前端 + 后端）
  ...
  - [ ] 20.2 实现小说聚合后端 API
    ...
    - 实现小说源适配器（至少 3 个初始源：笔趣阁/69书吧/Novel Updates 等）
...
- [ ] 21. 动漫聚合引擎（前端 + 后端）
  ...
  - [ ] 21.2 实现动漫聚合后端 API
    ...
    - 实现动漫源适配器（至少 3 个初始源：樱花动漫/GoGoAnime/AGE动漫等）
...
- [ ] 22. 直播聚合引擎（前端 + 后端）
  ...
  - [ ] 22.2 实现直播聚合后端 API
    ...
    - 实现直播源适配器（至少 3 个初始源：斗鱼/虎牙/Twitch 等）
...
- [ ] 23. 播客聚合引擎（前端 + 后端）
  ...
  - [ ] 23.2 实现播客聚合后端 API
    ...
    - 实现播客源适配器（至少 3 个初始源：Apple Podcasts/小宇宙/喜马拉雅等）
...
- [ ] 25. 成人视频聚合（前端 + 后端）
  ...
  - [ ] 25.2 实现成人视频聚合后端 API
    - 实现成人视频源适配器（通过 Cloudflare Workers 代理）
    - 所有成人视频源自动标记 NC-17 级
    - 实现多标签组合筛选 API
    - _需求: 17.1, 17.2, 17.3, 17.5, 17.8, 17.9_
...
- [ ] 26. 成人动漫聚合（前端 + 后端）
  ...
  - [ ] 26.2 实现成人动漫聚合后端 API
    - 实现成人动漫源适配器（Hanime.tv/HentaiHaven 等）
    - 所有成人动漫源自动标记 NC-17 级
    - _需求: 48.1, 48.8, 48.10, 48.11_
...
- [ ] 27. 成人漫画聚合（前端 + 后端）
  ...
  - [ ] 27.2 实现成人漫画聚合后端 API
    - 实现成人漫画源适配器（E-Hentai/nhentai 等）
    - 所有成人漫画源自动标记 NC-17 级
    - _需求: 19.1, 19.5, 19.8, 19.9_
...
- [ ] 29. 成人直播聚合（前端 + 后端）
  ...
  - [ ] 29.2 实现成人直播聚合后端 API
    - 实现成人直播源适配器（Chaturbate/StripChat 等）
    - 所有成人直播源自动标记 NC-17 级
    - _需求: 31.1, 31.5, 31.8, 31.9_
...
- [ ] 53. 成人游戏聚合（前端 + 后端）
  ...
  - [ ] 53.2 实现成人游戏聚合后端 API
    - 实现成人游戏源适配器（DLsite/Nutaku/Itch.io 成人区等）
    - 所有成人游戏源自动标记 NC-17 级
    - _需求: 34.1, 34.6, 34.9, 34.10_
